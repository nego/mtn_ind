             ...
             av_free_packet(&packet)) {
                ...
                if (0 == got_picture) {
                        ...
                        if (1000 == pkt_without_pic) { // is 1000 enough? // FIXME
                                av_log(NULL, LOG_INFO, "  * avcodec_decode_video2 couldn't decode picture\n");
                                av_free_packet(&packet);
                                return -1;
                        }
                } else {
                      ...
                      AVCodecContext __attribute__((unused)) *pCodecCtx, AVFrame __attribute__((unused)) *pFrame)
{
        ...
        double duration=(double) pFormatCtx->duration * av_q2d(pStream->time_base); // can be incorrect for .vob files
        if (duration > 0) {
                return duration;
        }
...
int really_seek(AVFormatContext *pFormatCtx, int index, int64_t timestamp, int flags, double duration)
{
        ...
        if (ret >= 0) { // success
                av_log(NULL, LOG_INFO, "AVSEEK_FLAG_ANY: timestamp: %"PRId64" (%.2f s)\n", timestamp, (timestamp / AV_TIME_BASE)); // DEBUG
                return ret;
        }
...
void make_thumbnail(char *file)
{
        ...
        if (NULL != parameters.gb_O_outdir && strlen(parameters.gb_O_outdir) > 0) {
                strcpy_va(tn.out_filename, 2, parameters.gb_O_outdir, "/");
...
//        pCodecCtx->skip_frame=AVDISCARD_NONREF; // internal err msg but not crash
        ...
        if (parameters.gb_x_duration > 1) {
                ...
                duration=parameters.gb_x_duration;
        } else {
        ...
        int64_t found_pts=-1;
        double found_target_seconds= found_pts * av_q2d(pStream->time_base);
        ...
        if (0 == ret) { // end of file
                av_log(NULL, LOG_INFO, "GOT EOF 4");
                goto eof;
        } else if (ret < 0) { // error
                av_log(NULL, LOG_INFO, "  read_and_decode first failed!\n");
                goto cleanup;
        }
        ...
        if (parameters.gb_C_cut > 0) {
                ...
                if (net_duration + parameters.gb_B_begin > duration) {
                        ...
                        av_log(NULL, LOG_INFO, "  -C %.2f s is too long, using %.2f s.\n", parameters.gb_C_cut, net_duration);
                }
        } else {
...
//        net_duration=duration - parameters.gb_B_begin - parameters.gb_E_end; // DVD
        ...
        int64_t rand_target_change=(int64_t)random_at_most((long)seek_target_step);
        //seek_target_step-=(rand_target_change / 10);
        double seek_target_step_seconds=seek_target_step * av_q2d(pStream->time_base);
...
//    }
        ...
        while (tn.shot_height < parameters.gb_h_height && tn.column > 0 && tn.shot_width != scaled_src_width) {
                ...
                if (parameters.gb_r_row > 0) {
                        ...
                        tn.row=floor(net_duration / tn.column / (seek_target_step * av_q2d(pStream->time_base)) + 0.5); // round nearest
                }
                if (tn.row < 1) {
                        tn.row=1;
                }
...
//        int evade_step=0;
        int evade_step=MIN(10, seek_target_step_seconds); // seconds to evade blank screen ; max 10 s        
...
restart: 
        ...
        int64_t last_seek_target=0, seek_target, seek_evade=0; // in time_base unit
        int evade_try=0; // blank screen evasion index
        double avg_evade_try=0; // average
        int direction=0; // seek direction (seek flags)
        int eof_one_count=0;
        int64_t better_step=0;
        int64_t prevshot_pts=0; // pts of previous good shot
        int64_t prevjpegshot_pts=0; // pts of previously saved JPEG
...
//    for (idx=0; idx < thumb_nb; idx++) {
        while (wedone == 0) {
                ...
                int64_t eff_target;
                if (near_end < 1) {
                        eff_target=seek_target + seek_evade; //effective target

                } else {
                        eff_target=seek_target; //effective target 
                }
...
//                }
                ...
                //usleep(100009);        
                /* if (prevshot_pts > eff_target && 0 == evade_try && real_idx % 3 == 0 && near_end < 1 && prevjpegshot_pts) { */
                ...
                eff_target=MAX(eff_target, prevjpegshot_pts + 1);
                if (eff_target > duration_tb) { // end of file
                        eff_target=(duration_tb - random_at_most(seek_target_step));
                        av_log(NULL, LOG_INFO, "GOT EOF 1 (DURATION: %.2f EFF_TARGET: %.2f)\n", net_duration, (eff_target * av_q2d(pStream->time_base)));
                        eof_one_count++;
                        if (eof_one_count > 10) goto eof;
                }
                ...
                if (1 == seek_mode) { // seek mode
                        ...
                        if (0 == ret) { // end of file
                                av_log(NULL, LOG_INFO, "GOT EOF 2");
                                goto eof;
                        } else if (ret < 0) { // error
                                av_log(NULL, LOG_INFO, "  read_and_decode failed!\n");
                                goto cleanup;
                        }
                        ...
                        while (found_pts < eff_target) {
                                ...
                                if (0 == ret && idx > parameters.gb_r_row - 1) { // end of file
                                        av_log(NULL, LOG_INFO, "GOT EOF 3");
                                        goto eof;
                                } else if (ret < 0) { // error
                                        av_log(NULL, LOG_INFO, "  read_and_decode failed!\n");
                                        goto cleanup;
...
//                av_log(NULL, LOG_INFO, "  found_diff: %.2f\n", found_diff); // DEBUG
                ...
                if (1 == seek_mode && 0 == parameters.gb_z_seek
                    ...
                    && (seek_target_step_seconds < 15 || found_diff > 15)
                    && (found_diff <= -seek_target_step_seconds || found_diff >= seek_target_step_seconds)) {

                        // compute the approx. time it take for the non-seek mode, if too long print a msg instead
                        double shot_dtime;
                        if (scaled_src_width > 576*4/3.0) { // HD
                                shot_dtime=seek_target_step_seconds * 30 / 30.0;
                        } else if (scaled_src_width > 288*4/3.0) { // ~DVD
                                shot_dtime=seek_target_step_seconds * 30 / 80.0;
                        } else { // small
                                shot_dtime=seek_target_step_seconds * 30 / 500.0;
                        }
...
//                rand_target_change=(int64_t)random_at_most((long)seek_target_step - 1);
                ...
                */
                ...
                if (evade_step > 0 && (blank > parameters.gb_b_blank || !is_edge(edge, EDGE_FOUND)) && 1 == 2) {
                        idx--;
                        evade_try++;
                        ...
                        if (seek_evade < ((seek_target_step - evade_step) / av_q2d(pStream->time_base))) { // FIXME
                                av_log(NULL, LOG_INFO, "  * blank or no edge * try #%d: seeking forward seek_evade: %"PRId64" (%.2f s)\n",
                                       evade_try, seek_evade, seek_evade  / AV_TIME_BASE);
...
//                        av_log(NULL, LOG_INFO, "%s : %d\n", idx, individual_filename, average_color);
                ...
                assert(NULL != individual_suffix);
                sprintf(individual_suffix, "_%013"PRId64".jpg", found_pts); // FIXME: hopefully 5 digits will be enough
                asprintf(&previous_suffix, "_%013"PRId64".jpg", found_pts); // FIXME: hopefully 5 digits will be enough

                if (is_reg(individual_filename) && dupe_count < 16 && shots_saved > 1) {

                        av_log(NULL, LOG_INFO, "%s (#%05d) already exists! (DUPECOUNT: %d)!\n", individual_filename, shots_saved, dupe_count);

                        rand_target_change=random_at_most(seek_target_step);
//                                seek_target=((seek_target_step * shots_saved + 1) - rand_target_change);
//                        seek_target += (seek_target_step / 10);
...
//                                found_pts -= rand_target_change;
                ...
                if (last_average_color < 15) {                
                        if (seek_target != last_average_color_time) {

                                average_color_square_size=((pCodecCtx->width + pCodecCtx->height) / 2); // average color of frame
                                average_color=get_average_color(ip, 1, 1, (average_color_square_size / 10));
...
//                                                                                if (average_color < MAXBLACK) {
                                ...
                                /* seek_target += (int64_t)random_at_most((long)seek_target_step/2); */
...
//                        if (found_pts - prevjpegshot_pts >= seek_target_step) {
                ...
                rand_target_change=(int64_t)random_at_most((long)seek_target_step - 1);
                if (  (real_idx - idx) < 5 && near_end < 1  && seek_target_step  > (found_pts - prevjpegshot_pts) && shots_saved > 2 /* && (found_pts - prevjpegshot_pts) > seek_target_step */ && bad_color < 1) {
...
//                        found_pts += rand_target_change;
                        idx--;
                        goto skip_shot;
                } else if (found_target_seconds < parameters.gb_B_begin && found_target_seconds > 0 && idx > 1) {
                        ...
                        if (!is_reg(individual_filename) && bad_color == 0){
                                av_log(NULL, LOG_INFO, "Saving shot: #%d @ %.2f to %s\n", shots_saved, ((found_pts) * av_q2d(pStream->time_base)), individual_filename);                
                                ...
                                prevjpegshot_pts=found_pts;
                                prevjpegshot_pts_seconds=found_pts * av_q2d(pStream->time_base);
                                shots_saved++;
                        }
                        ...
                        } else                         seek_target += (seek_target_step);

                } else if (near_end > 0 || (makepositive (seek_target - (shots_saved * seek_target_step))) > seek_target_step * 2) {
...
//                if (seek_target_step <= ( (4 * seek_target_step_original) / 5) && near_end < 1) seek_target_step=(seek_target_step_original);

                if (bad_color != 1) seek_target += random_at_most(seek_target_step);
...
//                seek_target -= (int64_t)random_at_most((long)seek_target_step - 1);
                ...
                if ((found_pts) > ((duration_tb / 20) * 17) && seek_target > prevjpegshot_pts) {
                        if (near_end==0){
                                near_end=1;                         
...
//                         seek_target_step -=                                 rand_target_change=(int64_t)random_at_most((long)(seek_target_step / 4)); */
//                         if (seek_target_step <= (seek_target_step_original / 10)) seek_target_step=(seek_target_step_original / 10); */
//                                                                seek_target -= seek_target_step; */
/* //                        } */
...
//                       seek_target += (int64_t)random_at_most((long)(seek_target_step));
                        /* av_log(NULL, LOG_INFO, "Adjusting seek_time from %lf to %lf\n", calc_time((seek_target + ((seek_target_step /5)*3)), pStream->time_base, start_time), calc_time(seek_target, pStream->time_base, start_time)); */
//                      if (seek_target > duration_tb) seek_target=duration_tb - (seek_target_step - (int64_t)random_at_most((long)seek_target_step - 1));
                }
                if (seek_target <= (shots_saved * seek_target_step_original * 3 / 4) && near_end < 1 && bad_color < 1) seek_target=shots_saved * seek_target_step_original;
                ...
                /*          av_log(NULL, LOG_INFO, "seek_target s: %.2f, eff_target s: %.2f, found_pts s: %.2f, duration s: %.2f\n", */
...
/* modified from glibc
 */
int _alphasort(const void *a, const void *b)
{
...
void process_dir(char *dir)
{
        ...
        qsort(v, cnt, sizeof(*v), _alphasort);
...
//        parameters.gb_v_verbose=1;
        ...
        while (-1 != (c=getopt(argc, argv, "a:b:B:c:C:D:e:E:f:F:g:h:iIj:k:L:nN:o:O:pPqr:s:tT:vVw:W:x:zZ"))) {
                switch (c) {
